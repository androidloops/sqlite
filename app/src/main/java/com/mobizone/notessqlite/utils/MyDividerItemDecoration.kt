package com.mobizone.notessqlite.utils
...
class MyDividerItemDecoration(private val context: Context, orientation: Int,
    ...
    private fun drawVertical(c: Canvas, parent: RecyclerView) {
        ...
        for (i in 0 until childCount) {
            ...
            val bottom = mDivider?.intrinsicHeight?.let { top + it }
            bottom?.let {
                mDivider?.setBounds(left + dpToPx(margin), top, right - dpToPx(margin), it)
                mDivider?.draw(c)
            }
        }
    }
    ...
    private fun drawHorizontal(c: Canvas, parent: RecyclerView) {
        ...
        for (i in 0 until childCount) {
            ...
            val right = mDivider?.intrinsicHeight?.let {
                left + it
            }
            right?.let {
                mDivider?.setBounds(left, top + dpToPx(margin), it,
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView,
        ...
        if (mOrientation == VERTICAL_LIST) {
            mDivider?.intrinsicHeight?.let {
                outRect.set(0, 0, 0, it)
            }
        } else {
            mDivider?.intrinsicWidth?.let {
